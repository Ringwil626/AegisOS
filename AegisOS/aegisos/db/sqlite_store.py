...
def init_db():
    ...
    # Projects table - Multi-project management support
    cursor.execute("""
    ...
    # Proposals table - Phase 6 evolution proposals (matches optimizer.py schema)
    cursor.execute("""
    ...
    # Strategy versions table (unified schema - matches strategy_manager.py)
    cursor.execute("""
    ...
    cursor.execute("""
        INSERT OR IGNORE INTO strategy_versions (version_tag, config_json, status)
        VALUES (?, ?, ?)
    """, ("v1.0-default", "{}", "active"))

    # Shadow runs table - for Phase 6 shadow testing (matches shadow_runner.py)
    cursor.execute("""
    ...
    # Insert default project (AegisOS itself)
    cursor.execute("""
        INSERT OR IGNORE INTO projects (name, path, description, status)
        VALUES (?, ?, ?, ?)
    """, ("aegisos", ".", "AegisOS Self-Management", "active"))
...
def get_pending_tasks_count() -> int:
    ...
    Used by Jung interface to display queue status.
    """
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
